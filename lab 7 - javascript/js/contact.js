...
function process() {
   ...
   // Validate the full name:
    var maxLength = 3;
    if (!fullname || !fullname.value || (fullname.value.indexOf('<') != -1)) {
        okay = false;
        alert('Please enter your name, with a space between each part of the name!');
    }
    else if (fullname.value.length > maxLength) {
        okay = false;
        var originalText = fullname.value;
       ...
       fullname.value = limitedText;
        okay = true;

        alert('Name was trimmed under ' + maxLength + ' characters');
    }
